...
public class FakeConnection extends Connection {
    ...
    public void send(Packet<?> obj, @Nullable PacketSendListener packetSendListener, boolean bl) {
        if (obj instanceof ClientboundSetEntityMotionPacket) {
            player.setDeltaMovement(player.getDeltaMovement(FakePlayer.MovementSide.SERVER), FakePlayer.MovementSide.CLIENT);
        } else if (obj instanceof ClientboundExplodePacket packet) {
            player.setDeltaMovement(player.getDeltaMovement(FakePlayer.MovementSide.CLIENT).add(packet.getKnockbackX(), packet.getKnockbackY(), packet.getKnockbackZ()), FakePlayer.MovementSide.CLIENT);
        }
    }
